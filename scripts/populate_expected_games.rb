...
teams = DB[:teams]
csv_file = File.expand_path(File.dirname(__FILE__)) + '/data/silver.csv'
...
CSV.foreach(csv_file, :headers => true) do |row|
  ...
  games = (row["ROUND OF 32"].to_f / 100.0).to_f + (row["ROUND OF 16"].to_f / 100.0).to_f + (row["ELITE 8"].to_f / 100.0).to_f + (row["FINAL 4"].to_f / 100.0).to_f + (row["FINALS"].to_f / 100.0).to_f + 1
